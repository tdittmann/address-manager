...
public class UpdateAddressCommandTest {
  ...
  public void testCreateAddress() throws ODataException {
    ...
    when(service
      .updateBusinessPartnerAddress(any(BusinessPartnerAddress.class))
      .execute()
    ).thenReturn(new ODataUpdateResult(new BasicHttpResponse(new ProtocolVersion("http", 1, 1), 200, "ok")));

    Integer result = new UpdateAddressCommand(service, munich).execute();
