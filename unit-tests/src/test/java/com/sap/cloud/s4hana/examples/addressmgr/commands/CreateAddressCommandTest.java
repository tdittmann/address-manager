...
public class CreateAddressCommandTest {
  ...
  public void testCreateAddress() throws ODataException {
    ...
    when(service
      .createBusinessPartnerAddress(any(BusinessPartnerAddress.class))
      .execute()
    ).thenReturn(munich);

    BusinessPartnerAddress result = new CreateAddressCommand(service, munich).execute();

    Assert.assertEquals("Germany", result.getCountry());
    Assert.assertEquals("778899", result.getCityCode());
    Assert.assertEquals("Munich", result.getCityName());
  }
